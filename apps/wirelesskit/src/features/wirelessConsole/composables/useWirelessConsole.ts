...
import { useLogStore } from "@/features/wirelessConsole/composables/useLogStore";
...
import { encodeText } from "@/domain/codec/text";
...
export function useWirelessConsole() {
  const { logs, level, keyword, filtered, push, clear } = useLogStore();
  ...
  async function btPickDevice(): Promise<void> {
    ...
    try {
      ...
      push("info", `已选择蓝牙设备：${btDeviceName.value}`);
    } catch (e) {
      const ue = toUserError(e);
      btError.value = ue.message;
      push("error", `选择设备失败：${ue.message}`, ue.detail);
    } finally {
      btBusy.value = false;
    }
  }
  ...
  async function btConnect(): Promise<void> {
    ...
    try {
      ...
      push("info", `已连接：${btDeviceName.value || "Bluetooth"}`);
      await btRefreshServices();
    } catch (e) {
      const ue = toUserError(e);
      btError.value = ue.message;
      push("error", `连接失败：${ue.message}`, ue.detail);
    } finally {
      btBusy.value = false;
    }
  }
  ...
  async function btDisconnect(): Promise<void> {
    ...
    try {
      ...
      const ue = toUserError(e);
      push("error", `断开失败：${ue.message}`, ue.detail);
    } finally {
      btBusy.value = false;
    }
  }
  ...
  async function btRefreshServices(): Promise<void> {
    try {
      ...
      push("debug", `Services：${svcs.length} 个`);
      await btRefreshCharacteristics();
    } catch (e) {
      const ue = toUserError(e);
      btError.value = ue.message;
      push("error", `读取 services 失败：${ue.message}`, ue.detail);
    }
  }
  ...
  async function btRefreshCharacteristics(): Promise<void> {
    ...
    try {
      ...
      push("debug", `Characteristics：${chrs.length} 个`);
    } catch (e) {
      const ue = toUserError(e);
      btError.value = ue.message;
      push("error", `读取 characteristics 失败：${ue.message}`, ue.detail);
    }
  }
  ...
  async function btRead(): Promise<void> {
    ...
    try {
      ...
      push("info", `Read：${v.hex}`);
    } catch (e) {
      const ue = toUserError(e);
      btError.value = ue.message;
      push("error", `Read 失败：${ue.message}`, ue.detail);
    } finally {
      btBusy.value = false;
    }
  }
  ...
  async function btWrite(): Promise<void> {
    ...
    try {
      ...
      push(
        "info",
        `Write（${btTxMode.value}）：${btTxMode.value === "hex" ? formatHex(bytes) : btTxText.value}`,
      );
    } catch (e) {
      const ue = toUserError(e);
      btError.value = ue.message;
      push("error", `Write 失败：${ue.message}`, ue.detail);
    } finally {
      btBusy.value = false;
    }
  }
  ...
  async function btStartNotify(): Promise<void> {
    ...
    try {
      ...
      push("info", "Notify：已开启");
    } catch (e) {
      const ue = toUserError(e);
      btError.value = ue.message;
      push("error", `Notify 开启失败：${ue.message}`, ue.detail);
    } finally {
      btBusy.value = false;
    }
  }
  ...
  async function nfcStart(): Promise<void> {
    ...
    try {
      ...
      push("error", `NFC 扫描失败：${ue.message}`, ue.detail);
    } finally {
      nfcBusy.value = false;
    }
  }
  ...
  async function nfcWrite(): Promise<void> {
    ...
    try {
      if (nfcWriteMode.value === "url") {
        ...
        push("info", `NFC 写入 URL：${nfcWriteUrl.value}`);
      } else {
        await nfc.writeText(nfcWriteText.value);
        push("info", `NFC 写入 Text：${nfcWriteText.value}`);
      }
    } catch (e) {
      const ue = toUserError(e);
      nfcError.value = ue.message;
      push("error", `NFC 写入失败：${ue.message}`, ue.detail);
    } finally {
      nfcBusy.value = false;
    }
  }
