...
export class Gd32SerialProtocol implements FlasherProtocol {
  ...
  async probe(): Promise<ProbeResult> {
    try {
      ...
      const idHex = Array.from(id)
        ...
        .join("");
      this.chipLabel = `GD32-${idHex}`;
      return { chipFamily: "gd32", chipName: this.chipLabel };
    } catch (cause) {
      if (this.connected) {
        try {
      ...
      throw this.mkError(ErrorCode.ProbeFailed, "串口握手/读取 GD32 芯片信息失败", cause);
    }
  }
  ...
  async buildPlan(input: unknown): Promise<FlashPlan> {
    ...
    if (!app) {
      throw this.mkError(
        ...
        "GD32 Serial 需要有效固件段",
        new Error("Invalid GD32 serial input"),
      );
    }
    return {
      chipFamily: "gd32",
  ...
  async erase(): Promise<void> {
    try {
      ...
      throw this.mkError(ErrorCode.EraseFailed, "擦除 GD32 Flash 失败", cause);
    }
  }
  ...
  async write(plan: FlashPlan, onProgress: (progress: StageProgress) => void): Promise<void> {
    ...
    try {
      ...
      this.connected = false;
      throw this.mkError(ErrorCode.FlashFailed, "写入 GD32 Flash 失败", cause);
    }
  }
  ...
  async reset(): Promise<void> {
    if (this.connected) {
      try {
        ...
        this.connected = false;
        throw this.mkError(ErrorCode.ResetFailed, "复位/断开 GD32 串口失败", cause);
      }
    }
  }
}
