...
export class Gd32DfuProtocol implements FlasherProtocol {
  ...
  async probe(): Promise<ProbeResult> {
    try {
      const device = this.transport.getDevice();
      await this.adapter.connect(device);
      return { chipFamily: "gd32", chipName: resolveGd32DfuChipName(device) };
    } catch (cause) {
  ...
  async buildPlan(input: unknown): Promise<FlashPlan> {
    ...
    if (!app) {
      throw toDownloadError(
        ...
        "GD32 USB DFU 需要有效固件段",
        new Error("Invalid GD32 USB DFU input"),
      );
    }
    return {
      chipFamily: "gd32",
