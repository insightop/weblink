...
系统 SHALL 提供持续扫描能力：会话在 READY 态可订阅设备的重复扫描结果，每次结果按名称合并（同名以最新为准）并按名称排序，直到订阅取消或会话关闭。订阅 SHALL 返回取消函数，取消后停止扫描轮询。

#### Scenario: 订阅持续扫描结果

- **WHEN** 上层订阅持续扫描，设备持续返回扫描结果
- **THEN** 每次结果回调合并、排序后的网络列表

#### Scenario: 取消扫描

- **WHEN** 上层调用订阅返回的取消函数或会话关闭
- **THEN** 停止扫描轮询，不继续触发回调

#### Scenario: 首次扫描失败回调 null

- **WHEN** 首次扫描失败（如设备不支持扫描）且此前未获得任何结果
- **THEN** 以 null 回调一次以表达"扫描不可用"，随后停止扫描
